...
public interface StudentMapper {
    ...
    @Insert("insert into student (username, password, name, phone, email, sex, birth, role) " +
            "values (#{username}, #{password}, #{name}, #{phone}, #{email}, #{sex}, #{birth}, #{role})")
    ...
    @Update("update student set username = #{username}, password = #{password}, name = #{name}, phone = #{phone}, " +
            "email = #{email}, sex = #{sex}, birth = #{birth} where id = #{id}")
